...
const Register = () => {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const updatedErrors: Errors = Object.assign(
        {},
      ...
      if (isFormValid) {
        ...
        const response = await fetch(
          `${envVars.NEXT_PUBLIC_BACK_URL}/users/register`,
          opciones,
        );
    ...
    <div className="my-16">
      ...
      >
        ...
        <input
          className={
            errors.name
              ? "border-2 border-red-600 w-full bg-red-200 p-2.5 rounded text-base mb-2.5"
              : "w-full p-2.5 border border-slate-300 rounded text-base mb-2.5"
          }
          type="text"
          value={formData.name}
          name="name"
        ...
        />
        {errors.address && (
          <p className="text-red-600 text-sm mb-4">{errors.address}</p>
        )}
        ...
        <button
          className={`${styles.button} flex justify-center items-center gap-4 shadow-md w-full border-slate-800 bg-[#1A1A1A] text-white text-sm font-medium mt-8 hover:-translate-y-1 transition p-3 duration-300 rounded border-2`}
        >
          Send
        </button>
